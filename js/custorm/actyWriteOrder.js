Zepto(document).ready(function() {
		...
		$.ajax({
			...
			success:function(res) {
				...
				if(res.code==0) {
					...
					// return;
					window.location.href = 'actyOrder_detail.html?orderId='+oId+'&item_type='+item_type+'';
				}
			},
			error:function() {
	...
	var onGobackTap = function() {
		window.location.href = 'local_activity_detail.html?tid='+zcHigoId+'&item_type='+item_type+'';
	};
	...
	var getPriceClendarData = function() {
		...
		var ajaxUrl = http_url+'/api/activity/pricedate';
		$.ajax({
			url:ajaxUrl,
			data:{actId:zcHigoId},
			success:function(res) {
	...
	var zcRenderDayPrice = function(day_str,zc_obj) { 
		...
		if(zc_obj[day_str]) {
			...
			return '<b class="curr_btg_info" style="display:block" data-stock='+tmpObj.stock+' data-day='+tmpObj.date+' data-childPrice='+tmpObj.childPrice+' >￥<span class="curr_day_price">'+tmpObj.price+'</span></b>'
