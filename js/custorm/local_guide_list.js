Zepto(document).ready(function() {
	...
	var getLineList = function(json_data,zc_dropload) { //TODO
		// var ajax_url = http_url+'/api/guide/listhtml';
		var ajax_url = http_url+'/api/guide/getlisthtml';
		var ajax_data_str =  '';
	...
	var renderPageByLocal = function(arr,return_data) {
		...
		$.each(arr,function(index, obj) {
			...
			list_arr.push(
					'<div class="evy_thm_wp" data-id='+obj.guideId+'>',
						'<div class="pic_wp"><img src='+obj.titlePic+'></div>',
						'<div class="itm_scan_wp">',
							'<div class="itm_title">'+obj.title+'</div>',
	...
	var onEvyThmTap = function() { //TODO跳转至详情页
		...
		zc_store.set('currTapCityCode',currTapCityCode);
		zc_store.set('currTapListJson',list_json);
		window.location.href = 'local_guide_detail.html?tid='+tid+'&item_type='+item_type+'';
	};
	...
	var getThemeTags = function() {
		var url = http_url+'/api/guide/tags';
		$.ajax({
			url:url,
			type:"GET",
			// dataType:'jsonp',
